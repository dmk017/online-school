...
async def test_update_user_id_validation_error(client):
    ...
    resp = await client.patch(f"/user/?user_id=123", json=user_data_updated)
...
async def test_update_user_id_not_found_error(client):
    user_data_updated = {
        "name": "Ivan",
        "surname": "Ivanov",
        "email": "cheburek@kek.com",
    ...
    user_id = uuid4()
    resp = await client.patch(f"/user/?user_id={user_id}", json=user_data_updated)
